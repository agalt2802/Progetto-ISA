document.getElementById("myButton").addEventListener("click", function () {
    ...
    // Make the request
    fetch("https://192.168.0.92:5443/picture")
  ...
  document.getElementById("resetCamera").addEventListener("click", function () {
    fetch("https://192.168.0.92:5443/reset?param=camera")
      .then((response) => {
        if (
          response.ok &&
  ...
  document.getElementById("savePhoto").addEventListener("click", function () {
    ...
    fetch("https://192.168.0.92:5443/save?param=camera")
      .then((response) => {
        if (
          response.ok &&
          response.headers.get("Content-Type").includes("text/html")
  ...
  function saveFile(){
    document.getElementById("confirm").addEventListener("click", function () {
      ...
      let url = "https://192.168.0.92:5443/createFile?fileName=" + encodeURIComponent(fileName)
      fetch(url)
        .then((response) => {
          if (
            response.ok &&
